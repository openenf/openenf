...
class AudioContextAnalyzeComponent {
    async analyze(resourceUri, preScanResult, expectedFrequency) {
        ...
        const targetFrequencies = (0, validatePrescanResult_1.validatePreScanResult)(preScanResult, expectedFrequency);
        let audioData;
        if (resourceUri instanceof Float32Array) {
            ...
            [audioData] = await (0, getAudioData_1.getAudioData)(buffer, resourceUri);
        }
        const analyzeProcessor = new goertzelAnalyzeProcessor_1.GoertzelAnalyzeProcessor(goertzelStore, targetFrequencies[0], this.overlapFactor, this.analyzeProgressEvent, audioData.length);

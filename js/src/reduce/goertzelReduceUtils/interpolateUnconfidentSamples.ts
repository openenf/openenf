import {HzWithConfidence} from "../../goertzel/GoertzelHarmonicResult";

const fixChunk = (s:HzWithConfidence[], chunk:{[id:number]:HzWithConfidence}) => {

    function fixStart(s:HzWithConfidence[], right:number) {
    ...
    function fixEnd(s:HzWithConfidence[], left:number) {
    ...
    function fixMid(s:HzWithConfidence[], left:number, right:number) {
    ...
    if (start == 0) {
        ...
        fixEnd(s, start)
    } else fixMid(s, start, end)
}

export const interpolateUnconfidentSamples = (samples:HzWithConfidence[], confidence:number, windowSize:number) => {
    if (!samples.length) {
        return samples;
    }
    confidence = confidence * (windowSize * windowSize);
    let interpolationChunk:{[id:number]:HzWithConfidence} = {};
    samples.forEach((s,i) => {

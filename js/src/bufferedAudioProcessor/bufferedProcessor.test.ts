...
describe('BufferedProcessor', () => {
    it('Executes processor when window size is exceeded', () => {
        const windowSize = 11;
        let executed = false;
        const subject = new BufferedAudioProcessor<number>(windowSize, (window) => {
            executed = true;
    ...
    it('Processes on quarter buffers if specified in constructor', () => {
        ...
        let processorExecutionCount = 0;
        const onWindowFull = (window:ArrayLike<number>) => {
            processorExecutionCount++;
            switch (processorExecutionCount) {
                case 1:
        ...
        const subject = new BufferedAudioProcessor<number>(windowSize, onWindowFull, overlapFactor);
